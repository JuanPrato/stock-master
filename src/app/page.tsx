...
import { getDashboardStatsWithOutCache } from "@/lib/stats.util";

export default async function Home() {

  const { stats, inventory, categories } = await getDashboardStatsWithOutCache();
  ...
  return (
    <main className="p-4 grid grid-cols-2 lg:grid-cols-4 gap-y-8 gap-x-4">
      <section className="col-span-2 lg:col-span-4 grid grid-cols-2 gap-4">
        <Card title="Total de Productos" detail={`+${stats.totalImproved} desde último mes`} icon={Package}
              value={stats.total}/>
        <Card title="Productos Bajos en Stock" detail="Necesitan reabastecimiento" icon={ClipboardList}
              value={stats.low}/>
        <Card title="Órdenes del día" detail={`${stats.dayUrgent} urgentes`} icon={ShoppingCart} value={stats.day}/>
        <Card title="Valor Total del Inventario" detail={`+${stats.totalValueImproved}% desde el último mes`}
              icon={DollarSign} value={stats.totalValue} isMoney/>
      </section>
      <Box>
